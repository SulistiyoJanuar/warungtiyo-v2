...
import { getAllProduct, saveItem } from "../utils/firebaseFunction";
...
const CreateContainer = () => {
  ...
  const uploadImage = (e) => {
    ...
    uploadTask.on(
      ...
      (error) => {
        console.log(error);
        setFields(true);
        setMsg("Eror while uploading : Try Again");
    ...
    setIsLoading(true);
    {
      try {
        if (!title || !imageAsset || !price || !category) {
          ...
          setIsLoading(false);
          setFields(true);
          setMsg("Data upload succesfully");
          clearData();
          setAlertStatus("succes");
          setTimeout(() => {
            setFields(false);
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center md:mt-0 -mt-10">
      <div className="w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        {fields && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`w-full p-2 rounded-lg text-center ${alertStatus === "danger" ? "bg-red-400 text-red-800" : "bg-emerald-400 text-emerald-800"}`}>
            {msg}
          </motion.p>
        )}

        <div className="w-full py-2 border-b border-gray-300 flex items-center gap gap-2">
        ...
        <div className="w-full">
          <select onChange={(e) => setCategory(e.target.value)} className="outline-none font-semibold w-full text-base border-b-2 border-gray-200 p-2 rounded-md cursor-pointer">
            <option value="other">Pilih Kategori</option>
            {categories &&
              categories.map((item) => (
                <option key={item.id} className="text-base border-0 outline-none capitalize bg-white text-black font-semibold" value={item.urlParamName}>
                  {item.name}
                </option>
              ))}
          </select>
        ...
        <div className="w-full h-full flex-col md:flex-row items-center gap-3">
          <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
            ...
            <input type="text" required value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Price" className="w-full h-full text-lg bg-transparent outline-none border-none placeholder:text-gray-400 " />
          </div>
        </div>
